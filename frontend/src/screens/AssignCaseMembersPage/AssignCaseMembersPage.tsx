  ...
  const assignLog = {
    ...
    action: `Assigned ${members.length} member(s) to case ${currentCaseId}`
  };

  // Append to "caseActivities" log
  const prevLogs = JSON.parse(localStorage.getItem("caseActivities") || "[]");
  const updatedLogs = [assignLog, ...prevLogs];
