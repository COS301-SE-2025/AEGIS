...
import { encryptMessage, decryptMessage,encryptBytes,decryptBytes,u8ToB64url } from '../../lib/crypto/aes_gcm'; // or your xchacha/secretbox helpers
...
async function fetchBundle(recipient: string): Promise<ServerBundleWire> {
  ...
  const promise = (async () => {
    ...
    const url = `http://localhost:8080/api/v1/x3dh/bundle/${id}`;
...
async function resolvePreviewUrl(url: string, mime: string): Promise<string> {
  if (!url || !/^https?:/i.test(url)) return url; // blob: or data: already fine
  try {
    const resp = await fetch(url, {
      // these two reduce CORS friction with gateways/CDNs that dislike referrers
      credentials: "omit",
      referrerPolicy: "no-referrer",
    });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const blob = await resp.blob();
    // if server gave no type, fall back to declared mime
    const typed = blob.type ? blob : new Blob([await blob.arrayBuffer()], { type: mime || "image/*" });
    const blobUrl = URL.createObjectURL(typed);
    trackBlobUrl?.(blobUrl); // so you can revoke later on unmount
    return blobUrl;
  } catch {
    // Fallback to original URL if fetch fails (still lets the user click to open)
    return url;
  }
}
