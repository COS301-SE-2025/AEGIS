...
func NewPostgresEvidenceRepository(db SQLDB) *PostgresEvidenceRepository {
...
func (repo *PostgresEvidenceRepository) GetEvidenceByCase(caseID string) ([]models.EvidenceDTO, error) {
    ...
    rows, err := repo.DB.Query(
        "SELECT id, case_id, uploaded_by, filename, file_type, ipfs_cid, file_size, checksum, metadata, uploaded_at FROM evidence WHERE case_id = $1", caseID)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func (repo *PostgresEvidenceRepository) GetEvidenceByID(evidenceID string) (*models.EvidenceDTO, error) {
    ...
    err := repo.DB.QueryRow(
        "SELECT id, case_id, uploaded_by, filename, file_type, ipfs_cid, file_size, checksum, metadata, uploaded_at FROM evidence WHERE id = $1", evidenceID).
...
func (repo *PostgresEvidenceRepository) SearchEvidence(query string) ([]models.EvidenceResponse, error) {
    q := "%" + query + "%"
    rows, err := repo.DB.Query(
        "SELECT id, filename, file_type, ipfs_cid FROM evidence WHERE filename ILIKE $1 OR file_type ILIKE $1 OR metadata::text ILIKE $1", q)
    if err != nil {
        return nil, err
    }
    defer rows.Close()
...
func (repo *PostgresEvidenceRepository) GetFilteredEvidence(caseID string, filters map[string]interface{}, sortField string, sortOrder string) ([]models.EvidenceResponse, error) {
    baseQuery := 
        "SELECT id, filename, file_type, ipfs_cid FROM evidence WHERE case_id = $1"  // <-- fixed space
